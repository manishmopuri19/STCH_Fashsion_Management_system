...
const UserManagement = () => {
  ...
  const handleUpdateUser = async (e) => {
    e.preventDefault();
    try {
      const response = await API.patch(`/users/${editingUser.id}`, newUser);
      setUsers((prev) =>
    ...
    <div className="min-h-screen bg-[#0B0F19] px-6 py-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-zinc-400 text-xs mt-0.5">Manage team access and roles</p>
          </div>
          <button
            onClick={() => {
              setIsEditMode(false);
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#2A3142] bg-[#1D2230]">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-widest">Designation</th>
                ...
                {users.length > 0 ? (
                  users.map((user) => (
                    <tr key={user.id} className="hover:bg-[#1D2230] transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-zinc-400">{user.designation || "—"}</td>
                      <td className="px-6 py-4 text-sm text-zinc-400">{user.phone || "—"}</td>
                      <td className="px-6 py-4">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#151821] border border-[#2A3142] rounded-3xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-[#2A3142] flex justify-between items-center sticky top-0 bg-[#151821] z-10">
            ...
            <form onSubmit={isEditMode ? handleUpdateUser : handleAddUser} className="p-6 space-y-4">
              <ModalInput
                label="User Name"
              ...
              <ModalInput
                ...
                placeholder="e.g. Merchandising, Finance"
              />
              <ModalInput
                label="Designation"
                value={newUser.designation}
                onChange={(v) => setNewUser({ ...newUser, designation: v })}
...
const ModalInput = ({ label, value, onChange, type = "text", placeholder, required }) => (
  <div className="flex flex-col">
